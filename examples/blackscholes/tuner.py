...
class ApproxTunerInterface(MeasurementInterface):
    ...
    def run_exec(self):
        run_cmd = ["./test.exec", "1", f"{TUNER}/examples/blackscholes/input.bin", f"{TUNER}/examples/blackscholes/output.bin"]
    ...
    def run(self, desired_result, input, limit):
        ...
        gcc_cmd = [
            ...
            f"{ROOT}/tools/cgeist/Test/approxMLIR/approx_blackscholes.mlir",
            "-pre-emit-transform",
            "-emit-approx",
            "-config-approx",
            "-transform-approx",
            "-o",
            f"{TUNER}/examples/blackscholes/test.mlir",
        ]
        ...
        cmd = [
            ...
            "-import-mlir",
            "-o",
            f"{TUNER}/examples/blackscholes/test.exec",
        ]
        ...
        with open("output.bin.txt", "r") as f:
            acc_data = f.read()
        time, out = parse_kernel_out(output, acc_data)
